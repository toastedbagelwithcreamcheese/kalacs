...
const PhotographySection = ({ title, description, images, extraImages }) => {
    ...
    >
      <div className="container mx-auto flex flex-col md:flex-row items-center gap-12 px-6">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-3xl md:text-4xl font-bold text-[#C79984]">{title}</h2>
      ...
      {selectedImage && (
        ...
        >
          <Image src={selectedImage} alt="Teljes képernyős kép" width={900} height={600} className="max-w-full max-h-full" />
        </motion.div>
      )}
    </motion.section>
  );
